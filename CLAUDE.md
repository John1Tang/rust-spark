...
- **Live pipelines are in-process tails.** A pipeline submitted with `refresh: live` (`Refresh::Live { poll_ms }`) polls its source forever and appends each batch to the destination file. The runner bypasses the planner for those flows (the polled batch IS the output), so `query` is ignored — a YAML whose `query: SELECT … FROM click_events` would otherwise be circular. `POST /v1/pipelines` returns `202 Accepted` with a status URL; status is at `GET /v1/pipelines/:name/status` and `GET /v1/pipelines/live`. The handle dies with the master pod, so `./scripts/seed-mock-data.sh` re-submits the live pipeline after every rolling restart.
- **Dashboard fallback ordering for `/examples/*`.** The dashboard's `axum::Router` merges the API router, then registers one explicit `route("/examples/e2e/demo_page.html", get(serve_demo_page))` for the embedded event-collector page, then `.fallback(dashboard_fallback)`. Adding new embedded static assets means a new `.route(...)` ahead of the fallback — otherwise the SPA fallback swallows them. The current single example (`examples/e2e/demo_page.html`) is embedded via `include_str!` in `crates/rspark-dashboard/src/assets.rs` so the image doesn't need a filesystem mount of `examples/`.
- **Demo page emits `{event_type, ts, url, user_id, country}`** — the live Kafka consumer doesn't normalize field names. The page's emitter (`examples/e2e/demo_page.html` script block) normalizes legacy `type`/`ts_ms` field names to `event_type`/`ts` before buffering so the on-disk NDJSON stays uniform. Mixing divergent schemas (e.g. some rows with `target:null` from a Python backfill) makes the JSON source's CSV reader fail with `record with 6 fields, but the previous record has 5 fields`. If you add a new producer, normalize its fields to the clickstream.jsonl shape before sending.
- **`LogicalPlan::collect_table_refs()`** (in `crates/rspark-sql/src/plan.rs`) returns the catalog tables referenced by any `Scan` in a plan tree, in first-occurrence order with duplicates removed. It's the primitive `/v1/sql` will use to decide whether to auto-spawn a live pipeline (queries touching any `TableKind::StreamingTable`). When you add a new `LogicalPlan` variant, also add an arm to `collect_table_refs_into` — the compiler won't catch this one.
- **`/v1/sql` sort ordering** — `ORDER BY col` where `col` is not in the SELECT list fails with `schema error: column '<col>' not found in schema`. The Sort node references the column from its child's schema (pre-Project), but the executor only sees the post-Project schema. Workaround for now: include the column in the SELECT list. Real fix (planner should add ORDER BY columns to the projection list internally, then drop them after sorting) is tracked as a follow-on.
- **Live NDJSON starts empty** after a fresh master pod. `./scripts/seed-mock-data.sh` re-points the catalog and restarts the live pipeline, but the destination file (`/tmp/rspark/live/click_events.ndjson`) is `/tmp`-resident and gets wiped. Either re-run the backfill (300 events from `examples/data/clickstream.jsonl`) or wait for the live pipeline to accumulate from page interactions.
